    ...
    case (Profiles.Current.ProfileType) of
      ptPDF: begin // PDF
        ...
        if (TPDFProfile (Profiles.Current).PageSize < PaperCount) then
          ...
          hsize := TPDFProfile (Profiles.Current).PageH / 1000;
          vsize := TPDFProfile (Profiles.Current).PageV / 1000;
          str (vsize:0:3, s);
          groffline += '-p' + s + 'p,';
          str (hsize:0:3, s);
          groffline += s + 'p'
...
{        if (DependCount > 0) then
      ...
      ptHTML : begin // HTML
        ...
        //if (Profiles.Current.Useeqn) then
        //  groffline += 'e';
        if (Profiles.Current.Usepreconv) then
          groffline += 'k';
        //if (Profiles.Current.Usetbl) then
        //  groffline += 't';
        groffline += ' ';
